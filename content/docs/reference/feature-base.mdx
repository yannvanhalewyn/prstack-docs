...
- The following branches are stacked on the feature base:
  - `api-auth → add-user-management` (authentication layer)
  - `api-users → api-auth` (user CRUD endpoints, stacked on auth)
  - `redesign-header → add-user-management` (header UI changes)
  - `redesign-footer → add-user-management` (footer UI changes)
  - `add-docs-for-user-management → add-user-management` (documentation)
- All work branches target the feature base instead of `main`
- When complete, `add-user-management` will have its own PR to merge into `main`
